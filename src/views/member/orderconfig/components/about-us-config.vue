<template>
  ...
  >
    ...
    <!-- 唯一字段：关于我们（富文本） -->
    <el-form-item label="关于我们" prop="aboutContent">
      <!-- 用户指定 <editor>，假设项目已注册/导入；v-model 绑定，min-height=400 -->
      <editor v-model="localForm.aboutContent" :min-height="400" />
    </el-form-item>
  </el-form>
</template>
  ...
  (newContent) => {
    if (newContent) {
      try {
        ...
        localForm.aboutContent = parsed.aboutContent || "";
      } catch (e) {
        ElMessage.error("解析配置失败");
        localForm.aboutContent = "";
      }
    }
  },
  { immediate: true }
  ...
  () => {
    emit("update:form", {
      ...
      content: JSON.stringify({
        aboutContent: localForm.aboutContent,
      }),
    });
  },
  { deep: true }
...
function handleSubmit() {
  aboutFormRef.value.validate((valid) => {
    if (valid) {
      emit("submit");
    }
  });

<!-- transaction-description-config.vue (子组件，全代码) -->
<template>
  <el-form
  ...
  >
    ...
    <el-form-item label="交易说明" prop="transactionDescription">
      ...
      <editor v-model="localForm.transactionDescription" :min-height="400" />
    </el-form-item>
  </el-form>
</template>
...
const localRules = reactive({
  transactionDescription: [
    { required: true, message: "请输入交易说明内容", trigger: "blur" },
  ...
  (newContent) => {
    if (newContent) {
      try {
        ...
        localForm.transactionDescription = parsed.transactionDescription || "";
      } catch (e) {
        ElMessage.error("解析配置失败");
        localForm.transactionDescription = "";
      }
    }
  },
  { immediate: true }
  ...
  () => {
    emit("update:form", {
      ...
      content: JSON.stringify({
        transactionDescription: localForm.transactionDescription,
      }),
    });
  },
  { deep: true }
...
function handleSubmit() {
  transactionFormRef.value.validate((valid) => {
    if (valid) {
      emit("submit");
    }
  });

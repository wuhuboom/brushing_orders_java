<!-- help-center-config.vue (子组件，全代码) -->
<template>
  <el-form
    ref="helpFormRef"
    :model="localForm"
  ...
  >
    <!-- 唯一字段：帮助中心（富文本） -->
    <el-form-item label="帮助中心" prop="helpContent">
      <!-- 用户指定 <editor>，假设项目已注册/导入；v-model 绑定，min-height=400 -->
      <editor v-model="localForm.helpContent" :min-height="400" />
    </el-form-item>
  </el-form>
</template>
...
const localRules = reactive({
  helpContent: [
    { required: true, message: "请输入帮助中心内容", trigger: "blur" },
  ...
  (newContent) => {
    if (newContent) {
      try {
        ...
        localForm.helpContent = parsed.helpContent || "";
      } catch (e) {
        ElMessage.error("解析配置失败");
        localForm.helpContent = "";
      }
    }
  },
  { immediate: true }
  ...
  () => {
    emit("update:form", {
      ...
      content: JSON.stringify({
        helpContent: localForm.helpContent,
      }),
    });
  },
  { deep: true }

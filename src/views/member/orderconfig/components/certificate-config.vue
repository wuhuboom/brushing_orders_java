<!-- certificate-config.vue (子组件，全代码) -->
<template>
  <el-form
  ...
  >
    <!-- 唯一字段：证书（富文本） -->
    <el-form-item label="证书" prop="certificateContent">
      <!-- 用户指定 <editor>，假设项目已注册/导入；v-model 绑定，min-height=400 -->
      <editor v-model="localForm.certificateContent" :min-height="400" />
    </el-form-item>
  </el-form>
</template>
  ...
  (newContent) => {
    if (newContent) {
      try {
        ...
        localForm.certificateContent = parsed.certificateContent || "";
      } catch (e) {
        ElMessage.error("解析配置失败");
        localForm.certificateContent = "";
      }
    }
  },
  { immediate: true }
  ...
  () => {
    emit("update:form", {
      ...
      content: JSON.stringify({
        certificateContent: localForm.certificateContent,
      }),
    });
  },
  { deep: true }

<template>
  <div class="app-container">
    ...
    <el-table :data="list" v-loading="loading" border stripe>
      ...
      <el-table-column prop="totalSubCount" label="下级总数" />
      <el-table-column prop="directInviteCount" label="直接邀请数" />
      ...
      <el-table-column prop="totalCommission" label="佣金总额">
      ...
      <el-table-column fixed="right" label="操作" width="160">
        <template #default="{ row }">
          <el-button type="primary" size="small" @click="openScopeUser(row)">查看下级用户</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    />
    <!-- 查看下级用户抽屉 -->
    <el-drawer title="下级用户" v-model="scopeOpen" direction="rtl" size="80%" append-to-body>
      <ScopeUser v-if="selectedUserId" :userId="selectedUserId" />
    </el-drawer>
  </div>
</template>
...
import ScopeUser from "@/views/member/member/ScopeUser.vue";
...
export default {
  ...
  setup() {
    ...
    function openScopeUser(row) {
      ...
      if (!id) {
        ElMessage.warning("无法确定该行的用户ID，无法查看下级");
        return;
      }
    ...
    return { filters, preset, dateRange, list, loading, onPresetChange, fetchStats, reset, formatAmount, total, queryParams, getList, scopeOpen, selectedUserId, openScopeUser };
  },
};
</script>

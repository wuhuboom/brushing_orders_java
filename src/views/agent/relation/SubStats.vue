<template>
  <div>
    ...
    <el-table :data="list" v-loading="loading" border stripe>
      ...
      <el-table-column fixed="right" :label="t('agent.operation')" width="160">
        <template #default="{ row }">
          <el-button type="primary" size="small" :disabled="!row.totalSubCount" @click="row.totalSubCount && openSubDrawer(row)">{{ t('agent.viewSubUsers') }}</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <!-- 嵌套下级用户抽屉 -->
    <el-drawer :title="t('agent.subUsersTitle')" v-model="subDrawerOpen" direction="rtl" size="80%" append-to-body>
      <SubStats v-if="subSelectedUserId" :userId="subSelectedUserId" />
    </el-drawer>
  </div>
</template>
...
import { subStats } from "@/api/member/member";
...
export default {
  ...
  setup(props) {
    const { t } = useI18n();
    const filters = ref({ beginTime: "", endTime: "" });
    ...
    const subSelectedUserId = ref(null);
    ...
    async function getList() {
      ...
      try {
        ...
        const res = await subStats(params);
    ...
    function openSubDrawer(row) {
      ...
      if (!id) return;
      subSelectedUserId.value = id;
      subDrawerOpen.value = true;
    }

    return { filters, preset, dateRange, list, loading, onPresetChange, fetchStats, reset, formatAmount, total, queryParams, getList, subDrawerOpen, subSelectedUserId, openSubDrawer, t };
  },
};
</script>

...
public class OrderTransactionalService {
    ...
    @Autowired
    private com.brushing.member.mapper.OrderGoodsHotelMapper orderGoodsHotelMapper;
    ...
    public AjaxResult createOrderTransactional(String username) {
        ...
        try {
            OrderInfo unfinishedOrder = orderInfoService.selectLatestUnfinishedOrder(user.getId());
            if (unfinishedOrder != null) {
                OrderGoods unfinishedGoods = selectGoodsByIdUsingSiteConfig(
                        unfinishedOrder.getProductId(), orderSiteConfig);
                OrderVoWrapper unfinishedOrderResponse = createOrderResponse(
                        unfinishedOrder,
                        unfinishedOrder.getProductId(),
                        unfinishedOrder.getPrice(),
                        unfinishedOrder.getCommission(),
                        unfinishedGoods == null ? null : unfinishedGoods.getName(),
                        unfinishedGoods == null ? null : unfinishedGoods.getCoverUrl(),
                        unfinishedOrder.getCreateTime(),
                        unfinishedOrder.getStatus());
                return new AjaxResult(907, "There is an open order", unfinishedOrderResponse);
            }
            ...
            try {
                if (!orderSeries.isEmpty() && series != null) {
                    ...
                    orderGoods = selectGoodsByIdUsingSiteConfig(productId, orderSiteConfig);
                    if (orderGoods == null) {
                        return AjaxResult.error(908, "Please try again later");
                    }
                    ...
                    setBalanceSnapshotAfterOrder(order, user, price);
                    ...
                    OrderGoods orderGoods = selectNearestGoodsByAmount(amount, orderSiteConfig);
                    if (orderGoods == null) {
    ...
    public AjaxResult submitOrderTransactional(Long id, String username) {
        OrderTradeControlConfig controlConfig = redisCache.getCacheObject("trade_config");
        // read site config once to determine which goods table to use
        OrderSiteConfig orderSiteConfig = siteConfigService.selectOrderSiteConfigById(1L);
        if (controlConfig == null) {
            return AjaxResult.error(901, "System configuration is not available");
        }
            ...
            try {
                if ("1".equals(orderInfo.getOrderType())) { // series
                    ...
                    if (series != null) { // create new series order
                        ...
                        OrderGoods orderGoods = selectGoodsByIdUsingSiteConfig(newSeriesOrder.getProductId(), orderSiteConfig);
    ...
    private void setBalanceSnapshotAfterOrder(OrderInfo order, OrderMemberUser user, BigDecimal price) {
        order.setBalance(user.getBalance().subtract(price));
        order.setFrozenBalance(user.getFrozenBalance().add(price));
    }
    ...
    private OrderInfo createNewSeriesOrder(OrderMemberUser user, OrderSeries series, int currentNum) {
        ...
        setBalanceSnapshotAfterOrder(newOrder, user, price);
    ...
    private OrderGoods selectGoodsByIdUsingSiteConfig(Long id, OrderSiteConfig siteConfig){
        if (siteConfig != null && "2".equals(siteConfig.getGoodsTableType())){
            return orderGoodsHotelMapper.selectOrderGoodsById(id);
        }
        return orderGoodsService.selectOrderGoodsById(id);
    }

    private OrderGoods selectNearestGoodsByAmount(BigDecimal amount, OrderSiteConfig siteConfig){
        if (siteConfig != null && "2".equals(siteConfig.getGoodsTableType())){
            return orderGoodsHotelMapper.selectNearestPriceGoods(amount);
        }
        return orderGoodsService.selectNearestPriceGoods(amount);
    }
 }

...
test('member list keeps the reference table viewport and status prompts describe the target state', () => {
  ...
  assert.match(member, /columnWidth: 32/)
  assert.match(member, /flex: 1 1 0 !important;/)
  assert.match(member, /font-size: 15px;/)
  assert.match(member, /border-bottom: 1px solid #f0f0f0 !important;/)
  assert.equal(
...
test('member management keeps root-member hierarchy and legacy actions aligned', () => {
  ...
  const payload = source('src/views/member/orderuser/components/orderuserFormPayload.js')

  assert.match(form, /顶级用户/)
  assert.match(form, /下级用户/)
  assert.match(payload, /payload\.parentId\s*=\s*parentInviteCode\s*\?\s*\(payload\.parentId\s*\?\?\s*null\)\s*:\s*0/)
  assert.match(payload, /payload\.parentInviteCode\s*=\s*parentInviteCode\s*\|\|\s*null/)
  ...
  assert.match(member, /:trigger="\['hover'\]"/)
  ...
  assert.match(member, /tradePassword:\s*String\(tradeForm\.tradePassword \|\| ""\)\.trim\(\)/)
...
test('website settings mirror the reference list and large drawer layout', () => {
  ...
  for (const field of ['enUs', 'esEs', 'frFr', 'zhCn', 'deDe', 'itIt', 'koKr', 'jaJp', 'svSe', 'noNo', 'ruRu', 'huHu', 'plPl', 'skSk']) {
    assert.match(adminLanguageFields, new RegExp(`"${field}"`))
  }
  assert.doesNotMatch(translationLanguages, /field:\s*"idId"|column:\s*"id_ID"|"idId"/)
...
test('level translations expose newly requested languages supported by the backend', () => {
  ...
  assert.doesNotMatch(level, /"idId"/)
})
...
test('customer service management mirrors the reference list and drawer fields', () => {
  ...
  assert.match(customerService, /root-class-name="customer-service-drawer"/)
  ...
  assert.match(customerService, /:status="String\(record\.isEnabled\) === '1' \? 'success' : 'error'"/)
...
test('withdrawal type management mirrors the reference drawer and parameter fields', () => {
  ...
  assert.match(withdrawalType, /root-class-name="withdrawal-type-drawer"/)
  ...
  assert.match(withdrawalType, /\{ label: "银行卡", value: "1" \}[\s\S]*?\{ label: "网络", value: "2" \}/)
...
test('financial audit lists match the legacy columns, filters and page size', () => {
  ...
  assert.doesNotMatch(recharge, /title:\s*"转换后金额"/)
  ...
  assert.match(recharge, /title:\s*"操作"[\s\S]*?width:\s*150/)
  ...
  assert.doesNotMatch(withdrawal, /title:\s*"转换后金额"/)
  ...
  assert.match(withdrawal, /账户持有人/)
  assert.match(withdrawal, /银行账号/)
...
test('level management mirrors the reference search, table and drawer structure', () => {
  ...
  assert.match(level, /title:\s*"最低连单返佣百分比"/)
  assert.match(level, /:scroll="\{ x: 3600/)
  assert.match(level, /width="70%"/)
  assert.match(level, /size="large"/)
  assert.doesNotMatch(level, /prop:\s*"signInBonus"/)
  assert.doesNotMatch(level, /prop:\s*"version"/)
})
